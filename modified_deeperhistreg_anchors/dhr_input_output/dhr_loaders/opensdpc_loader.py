...
class OpenSdpcLoader(WSILoader):
    """
    TODO - documentation
    """
    def __init__(
        ...
        self.mode = mode
        self.image = opensdpc.OpenSdpc(self.image_path)
        # pyvips.Image.openslideload(self.image_path) # default level 0
        self.num_levels = self.get_num_levels()
        self.resolutions = self.get_resolutions()
        # print('res_level0', self.resolutions[0])
        # print(self.resolutions)
        # level0_dim (40320, 24192) (99456, 29568)
        
    def get_num_levels(self) -> int:
        """
        TODO - documentation
        """       
        return int(self.image.level_count)
        # return int(self.image.get('openslide.level-count'))
    
    def load_pyvips_sdpc_level(self, level):
        # w, h, b = self.resolutions[level]
        h, w, b = self.resolutions[level] 
        # print('w:', w, 'h:', h)
        region = self.image.read_region((0, 0), level, (w, h))
        array = np.array(region)
        # print('array:', array.shape)
        return pyvips.Image.new_from_memory(
                array.tobytes(), w, h, array.shape[2], "uchar")
    ...
    def get_resolutions(self) -> Iterable[int]:
        ...
        for level in range(self.num_levels):
            ...
            width, height = self.image.level_dimensions[level]
            bands = 3
            # slide.level_dimensions[0] == (image.width, image.height)
    ...
    def get_best_level(self, resample_ratio : float) -> pyvips.Image:
        ...
        if self.num_levels == 1:
            ...
            # slide.level_dimensions[0] == (image.width, image.height)
            org_width, org_height = self.image.level_dimensions[0]
            # org_height, org_width = self.image.level_dimensions[0]
            ...
            return self.load_pyvips_sdpc_level(current_level), current_level
    ...
    def resample(self, resample_ratio : float) -> Union[np.ndarray, tc.Tensor]:
        ...
        # print('best_level_img', 'w', image.width, 'h', image.height)
        if level_to_use > 0:
    ...
    def load_region(self, level, offset, shape) -> Union[np.ndarray, tc.Tensor]:
        ...
        image = self.load_pyvips_sdpc_level(level)
        # image = pyvips.Image.openslideload(self.image_path, level=level)
