...
import modified_deeperhistreg_anchors.dhr_registration.dhr_initial_alignment.superpoint_superglue_patch as sg
import torch
...
def multi_feature_patch(
    ...
    if initial_resample:
        ...
        print('shape in rigid registartion', source.shape, target.shape)
    ...
    for angle in range(angle_start, angle_stop, angle_step):
        ...
        # "run_sift_ransac": true, "run_superpoint_superglue": true, "run_superpoint_ransac": false
        ...
        if run_sift_ransac:
            ...
            # --------------------------------------------------------
            source_keypoints = source_keypoints.squeeze() # (N, 1, 2)
            target_keypoints = target_keypoints.squeeze()
            # --------------------------------------------------------
        ...
        if run_superpoint_superglue:
            ...
            # print('in superpoint, resampled_source', resampled_source.shape)
