...
def sift_ransac(
    ...
    # source_points, target_points, matches = matcher(source_keypoints, target_keypoints, source_descriptors, target_descriptors) # len 0
    try:
    ...
    source_pts = np.array([kp.pt for kp in source_keypoints])
    target_pts = np.array([kp.pt for kp in target_keypoints])
    if return_num_matches:
        return final_transform, num_matches, source_points, target_points, resampled_source
    else:
        return final_transform, source_points, target_points, resampled_source
