...
def initial_resampling(
    ...
    # F.interpolate(tensor, scale_factor = 1 / resample_ratio, mode=mode, recompute_scale_factor=False, align_corners=False)
    return resampled_source, resampled_target

def initial_resampling_fix_reg(
    source : Union[tc.Tensor, np.ndarray],
    target : Union[tc.Tensor, np.ndarray],
    resolution : int) -> Tuple[Union[tc.Tensor, np.ndarray], Union[tc.Tensor, np.ndarray]]:
    """
    防止F.interpolate 的向下取整机制导致达不到reg_size
    """
    source_y_size, source_x_size, target_y_size, target_x_size = get_combined_size(source, target)
    resample_ratio = calculate_resampling_ratio((source_x_size, target_x_size), (source_y_size, target_y_size), resolution)
    resampled_source = resample_fix_reg(gaussian_smoothing(source, min(max(resample_ratio -1, 0.1), 10)), resample_ratio, resolution)
    resampled_target = resample_fix_reg(gaussian_smoothing(target, min(max(resample_ratio -1, 0.1), 10)), resample_ratio, resolution)

    factor = 1/resample_ratio
    # print('resample_ratio', resample_ratio, 'factor', factor)
    # print('source', source.shape, source.shape[0] * factor, source.shape[1] * factor)

    # F.interpolate(tensor, scale_factor = 1 / resample_ratio, mode=mode, recompute_scale_factor=False, align_corners=False)
    return resampled_source, resampled_target

def resample_fix_reg(tensor : tc.Tensor, resample_ratio : float, reg_size: int, mode: str="bilinear") -> tc.Tensor:
    """
    To prevent the rounding down mechanism of F.interpolate from causing the reg_size to not be reached.
    """
    factor = 1 / resample_ratio
    # print(tensor.shape)
    new_size = [tensor.shape[2]*factor, tensor.shape[3]*factor ]
    # (tensor.shape[2]*factor, tensor.shape[3]*factor)
    # print('new_size', new_size)
    
    if abs(new_size[0] - reg_size)<0.5:
        new_size[0] = reg_size
        new_size[1] = int(new_size[1])
    elif abs(new_size[1] - reg_size)<0.5:
        new_size[1] = reg_size
        new_size[0] = int(new_size[0])
    
    # print('new_size1', new_size)
    return F.interpolate(tensor, new_size, mode=mode, recompute_scale_factor=False, align_corners=False)
